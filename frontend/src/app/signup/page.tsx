...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f7faf8] flex items-center justify-center px-4 relative overflow-hidden">
      <div className="absolute top-0 right-1/3 w-[500px] h-[500px] bg-[#dae2fd]/30 rounded-full blur-[120px]" />
      <div className="absolute bottom-0 left-1/4 w-[400px] h-[400px] bg-[#005c55]/5 rounded-full blur-[100px]" />
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <div className="p-8 rounded-2xl bg-white border border-[#bdc9c6] shadow-sm card-elevated">
          <h1 className="text-2xl font-bold text-[#0f172a] text-center mb-1">Create Your Account</h1>
          <p className="text-[#6e7977] text-sm text-center mb-8">Start your AI-powered career journey</p>

          {error && <div className="mb-4 p-3 rounded-xl bg-[#ffdad6] border border-[#ba1a1a]/20 text-[#93000a] text-sm">{error}</div>}

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[#181c1c] mb-1.5">Full Name</label>
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6e7977]" />
            ...
            <div>
              <label className="block text-sm font-medium text-[#181c1c] mb-1.5">Confirm Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6e7977]" />
        ...
        <p className="mt-6 text-center text-sm text-[#6e7977]">
          Already have an account? <Link href="/login" className="text-[#005c55] hover:text-[#0f766e] font-medium">Sign in</Link>
        </p>
      </motion.div>
    </div>
  );
